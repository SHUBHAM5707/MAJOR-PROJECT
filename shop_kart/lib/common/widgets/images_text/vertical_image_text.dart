import 'package:flutter/material.dart';

import '../../../utils/constants/colors.dart';
import '../../../utils/constants/sizes.dart';
import '../../../utils/helpers/helper_function.dart';
class SkVerticalImageText extends StatelessWidget {
  const SkVerticalImageText({
    super.key,
    required this.image,
    required this.title,
    this.textColor = SkColors.white,
    this.backgroundColor,
    this.onTap,
  });

  final String image, title;
  final Color textColor;
  final Color? backgroundColor;
  final void Function()? onTap;

  @override
  Widget build(BuildContext context) {
    final dark = SKHelperFunction.isDarkMode(context);

    return GestureDetector(
      onTap: onTap,
      child: Padding(
        padding: const EdgeInsets.only(right: SkSizes.spaceBtwIteam),
        child: Column(
          children: [
            ///--circular icon
            Container(
              width: 56,
              height: 56,
              padding: const EdgeInsets.all(SkSizes.sm),
              decoration: BoxDecoration(
                color: backgroundColor ?? (dark ? SkColors.black : SkColors.white),
                borderRadius: BorderRadius.circular(100),
              ),
              child: Center(
                child: Image(
                    image: AssetImage(image), fit: BoxFit.cover, color: dark ? SkColors.light : SkColors.dark),
              ),
            ),

            ///Text
            const SizedBox(height: SkSizes.spaceBtwIteam / 2),
            SizedBox(
              width: 55,
              child: Text(
                title,
                style: Theme.of(context)
                    .textTheme
                    .labelMedium!
                    .apply(color: textColor),
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
              ),
            )
          ],
        ),
      ),
    );
  }
}
